...
function Section({ type, label }) {
  ...
  const handleSubmit = async e => {
    ...
    const res = await fetch(LegalAssistantConfig.restUrl + '/analyze', { method: 'POST', body: formData });
    ...
    <div className="border p-4 bg-white rounded shadow">
      ...
      {result && (
        <div className="mt-4">
          ...
          {result.file && (
            <p className="text-sm mt-1">Файл сохранён как: <a className="text-blue-600 underline" href={`${LegalAssistantConfig.uploadsUrl}/${result.file.filename}`} target="_blank" rel="noopener noreferrer">{result.file.originalname}</a></p>
          )}
...
function FilesList() {
  ...
  useEffect(() => {
    fetch(LegalAssistantConfig.restUrl + '/files')
      .then(res => res.json())
    ...
    <div className="mt-8 border p-4 bg-white rounded shadow">
      ...
      {files.length === 0 ? (
        ...
        <ul className="list-disc pl-5 space-y-1">
          {files.map(f => (
            <li key={f}>
              <a className="text-blue-600 underline" href={`${LegalAssistantConfig.uploadsUrl}/${f}`} target="_blank" rel="noopener noreferrer">{f}</a>
            </li>
          ))}
        </ul>
      )}
...
function PagesList() {
  ...
  useEffect(() => {
    fetch(LegalAssistantConfig.restUrl + '/pages')
      .then(res => res.json())
...
function PageView() {
  ...
  useEffect(() => {
    fetch(`${LegalAssistantConfig.restUrl}/pages?id=${id}`)
      .then(res => res.json())
